<template>
  <div style="padding:5px;">
    <Button style="margin-right: 4px;" type="primary" shape="circle" icon="ios-refresh" @click="handleRefresh"
            :disabled="loading">{{ $t('Refresh List') }}
    </Button>
    <Input style="margin-right: 4px;width:200px" v-model="filterString" search @input="handleSearch"
           :placeholder="$t('Please enter the filter key...')"/>
    <Button style="margin-right: 4px;" @click="showCmdForm" type="primary" :disabled="cmdStatus.execing > 0">
      {{ $t('Execute command') }}
    </Button>
    <Button v-if="isadmin" style="margin-right: 4px;" @click="showBindForm" type="primary">{{
        $t('Bind user')
      }}
    </Button>
    ...
    <div style="float: right; margin-right: 50px">
      <span style="margin-right: 20px; color: #3399ff; font-size: 24px">{{
          $t('device-count', {count: onlineTotal})
        }}</span>
    ...
    <Table :loading="loading" :columns="columnsDevices" :data="filteredDevices" style="margin-top: 10px; width: 100%"
           :no-data-text="$t('No devices connected')" @on-selection-change='handleSelection'>
      ...
      <template v-slot:action="{ row }">
        <Button v-if="isadmin && row.bound" type="warning" size="small" style="vertical-align: bottom;"
                @click="unBindUser(row.id)">{{ $t('Unbind') }}
        </Button>
        <Tooltip v-if="row.online" placement="top" :content="$t('Access your device\'s Shell')">
          <i class="iconfont icon-shell" style="font-size: 40px; color: black; cursor:pointer;"
             @click="connectDevice(row.id)"/>
        </Tooltip>
        <Tooltip v-if="row.online" placement="top" :content="$t('Access your devices\'s Web')">
          <i class="iconfont icon-web" style="font-size: 40px; color: #409EFF; cursor:pointer;"
             @click="connectDeviceWeb(row)"/>
        </Tooltip>
        <span v-if="!row.online" style="margin-left: 10px; color: red">{{ $t('Device offline') }}</span>
      </template>
    </Table>
    <Page :total="total" :page-size="limit" @on-change="changePage" @on-page-size-change="_nowPageSize"
          show-total/>
    <Modal v-model="cmdModal" :title="$t('Execute command')" @on-ok="doCmd">
      <Form ref="cmdForm" :model="cmdData" :rules="cmdRuleValidate" :label-width="100" label-position="left">
        ...
        <FormItem :label="$t('Parameter')" prop="params">
          ...
          <Button v-else style="width: 40px; margin-left: 10px;" size="small" icon="ios-add" type="primary"
    ...
    <Modal v-model="cmdStatus.modal" :title="$t('Status of executive command')" :closable="false"
           :mask-closable="false">
      ...
      <div slot="footer">
        <Button type="primary" size="large" :disabled="cmdStatus.execing > 0" @click="showCmdResp">{{
            $t('OK')
          }}
        </Button>
        <Button type="error" size="large" :disabled="cmdStatus.execing === 0" @click="ignoreCmdResp">{{
            $t('Ignore')
          }}
        </Button>
      </div>
...
export default {
  ...
  methods: {
    ...
    getDevices() {
      this.axios.post('/devs', {page: this.page, limit: this.limit, keyWord: this.filterString}).then(res => {
        this.loading = false;
        this.devlists = res.data.list;
        this.filteredDevices = res.data.list;
        this.selection = [];
