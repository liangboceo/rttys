...
func apiStart(br *broker) {
	...
	authorized.POST("/devs", func(c *gin.Context) {
		devReq := DevReq{}
		_ = c.ShouldBind(&devReq)
		if devReq.Page <= 0 {
			devReq.Page = 1
		}
		if devReq.Limit <= 0 {
		...
		//搜索分页查询
		if devReq.KeyWord != "" {
			devReq.KeyWord = "%" + strings.TrimSpace(devReq.KeyWord) + "%"
			if searchUserName != "" {
				sql = fmt.Sprintf("SELECT id, description, username FROM device WHERE username = '%s' AND  (description LIKE '%s' or id LIKE '%s') limit %d,%d ", searchUserName, strings.ToLower(devReq.KeyWord), strings.ToLower(devReq.KeyWord), startRows, devReq.Limit)
				countSql = fmt.Sprintf("SELECT count(id) FROM device WHERE username = '%s' AND  (description LIKE '%s' or id LIKE '%s') ", searchUserName, strings.ToLower(devReq.KeyWord), strings.ToLower(devReq.KeyWord))
			} else {
				sql = fmt.Sprintf("SELECT id, description, username FROM device WHERE (description LIKE '%s' or id LIKE '%s') limit %d,%d ", strings.ToLower(devReq.KeyWord), strings.ToLower(devReq.KeyWord), startRows, devReq.Limit)
				countSql = fmt.Sprintf("SELECT count(id) FROM device WHERE (description LIKE '%s' or id LIKE '%s') ", strings.ToLower(devReq.KeyWord), strings.ToLower(devReq.KeyWord))
			}
		} else {
			if searchUserName != "" {
				sql = fmt.Sprintf("SELECT id, description, username FROM device WHERE username = '%s'  LIMIT %d,%d ", searchUserName, startRows, devReq.Limit)
				countSql = fmt.Sprintf("SELECT count(id) FROM  devic  WHERE username = '%s'", searchUserName)
			} else {
				sql = fmt.Sprintf("SELECT id, description, username FROM device LIMIT %d,%d ", startRows, devReq.Limit)
				countSql = "SELECT count(id) FROM device"
			}
		...
		_ = db.QueryRow(countSql).Scan(&devListResp.Total)
		devListResp.OnlineTotal = len(br.devices)
